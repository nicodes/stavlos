...
Old tool results are cleared from what a model is sent once newer ones hold `compaction.clearTokens` (40,000 by default) worth: the call stays on record and the file can be read again, so this is free and lossless, and it is most of what a long history holds. Context is compacted on its own when an agent's history passes `compaction.maxTokens` (150,000 by default: a budget, not a share of the window, since a million-token window is a reason to allow that many, not to carry them on every call; `-1` uses the window alone) or 80% of its model's window, whichever comes first (the provider's own count of the last call is used when it is larger than the estimate): older turns become a summary, and the most recent conversation that fits in `keepTokens` (15,000 by default) is kept beside it.
