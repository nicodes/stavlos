...
The TUI opens immediately. With nothing configured, the first prompt answers "no model selected"; run `/provider` to paste a key and `/models` to pick a model. The first model you pick becomes your default for future sessions.
...
In the TUI: type to talk to the selected agent (`/role` switches which preset it runs as, `/roles` lists them); if it's busy, your message reaches it at its next step. `/queue <text>` waits for the current turn to end instead, and `/cancel` ends the current turn. Delegating to and killing child agents is the model's job (`agent_create`, `agent_kill`); a finished child wakes its parent between turns, never mid-turn, and there is no wait tool. Agents run slow commands with `bash_async`, whose exit wakes them the same way; live child agents and jobs get their own tabs ("agents" and "async") beside the "permission" tab in a strip above the input (all three are always there with a count, and the focused one opens). Tab and shift+tab cycle focus top to bottom between the chat, agents, async, permission, the input, and the sidebar (ctrl+b). In the chat, up/down move item by item and Enter expands a tool call's output; in the input, up/down walk your prompt history. `/help` lists the rest. Permission prompts open from their tab above the input; `y` allows once, `a` allows for the session, `n` denies.
