...
**Delegate and message.** Delegating to child agents is the model's job (`agent_create`). Every agent has a unique name in its session (the root is `main`; a name already taken gets a suffix, `scout-2`), and agents talk with one tool, `message`, addressed by name to any other agent or to you as `user`. To an agent waiting on the sender, such as a parent waiting on its child's task, a message is the answer: it wakes that agent between turns, never mid-turn. To any other agent it is a new message that reaches it at its next step, even mid-turn, and the sender waits for the answer. A message sent with `no_reply` (thanks, an acknowledgement) needs no answer: nobody owes a reply or waits, and an idle recipient is not woken for it. A role can keep its agents from messaging you with a deny rule on `message` for `user`. A child idles with its context intact for follow-ups for the rest of the session: nothing kills it, and its MCP servers stop after ten idle minutes. There is no wait tool.
