...
Stavlos uses your existing subscription, not platform API keys. `/provider` offers two sign-ins: **ChatGPT** (Plus or Pro, through the Codex sign-in) and **Grok** (SuperGrok, through the Grok CLI sign-in). Each shows a URL and a short code; open the URL on any device, enter the code, and you're connected. Tokens live in `~/.local/share/stavlos/auth.json` (mode 0600) and refresh automatically. `/models` picks a model; the first pick becomes your default. On the command line, `stavlos auth login [openai|xai]`, `stavlos auth list`, and `stavlos auth logout` do the same.
