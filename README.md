...
Stavlos uses your existing subscription, not platform API keys. `/providers` offers three sign-ins:
...
Credentials live in `~/.local/share/stavlos/auth.json` (mode 0600); OAuth tokens refresh automatically, and a key does not expire. On the command line, `stavlos auth login [openai|xai|zai]`, `stavlos auth list` and `stavlos auth logout` do the same. Model ids are `openai/gpt-5.4`, `xai/grok-4`, `zai/glm-5.3`, and so on; `/models` lists what each subscription serves.
