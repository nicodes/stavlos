...
Old tool results are cleared from what a model is sent once newer ones hold `compaction.clearTokens` (40,000 by default) worth, never from the two most recent turns and never a message, a child's answer or the human's reply: the call stays on record and the file can be read again, so this is free and lossless, and it is most of what a long history holds. A tool output over `compaction.maxToolOutput` (50 KB) is cut in the middle and kept whole in the channel's scratch directory, which the result names, so the agent reads the part it needs instead of running the command again. Context is compacted on its own when an agent's history passes 90% of what its model's window leaves after the reply's reserve (`compaction.threshold`; OpenCode and Codex draw the line the same way), or `compaction.maxTokens` if you set a budget, whichever comes first (the provider's own count of the last call is used when it is larger than the estimate): older turns become a summary, and the most recent conversation that fits in `keepTokens` (15,000 by default) is kept beside it.
