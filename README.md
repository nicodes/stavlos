...
In the TUI: type to prompt the selected agent. `/steer <text>` redirects it mid-turn, `/cancel` ends the current turn, `/kill` tears an agent down, `/spawn <archetype> <label> <task>` delegates (children report to the parent's mailbox and never interrupt it; `monitor` arms a wake, `unmonitor` disarms it, and there is no blocking wait), Tab cycles agents, `/help` lists the rest. Permission prompts appear above the input; `y` allows once, `a` allows for the session, `n` denies.
