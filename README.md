...
Context is compacted on its own when an agent's history passes 80% of its model's window, or `"compaction": {"maxTokens": 200000}` if you set one, whichever comes first (the provider's own count of the last call is used when it is larger than the estimate): older turns become a summary, and the most recent conversation that fits in `keepTokens` (15,000 by default) is kept beside it.
...
Set `STAVLOS_KEEP_DAEMON=1` to skip that.
