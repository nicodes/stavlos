...
In the TUI: type to prompt the selected agent. `/steer <text>` redirects it mid-turn, `/cancel` ends the current turn, `/kill` tears an agent down, `/spawn <archetype> <label> <task>` delegates (the agent-side tool is `agent_create`) (a finished child wakes its parent between turns, never mid-turn; `monitor` waits, `unmonitor` opts out, and there is no blocking wait). Agents can also run a command in the background, watch a path, or set a timer; those are monitors, listed separately from child agents above the input, Tab and shift+tab cycle focus between the chat, a pending permission prompt, the input, and the sidebar (ctrl+b). In the chat, up/down move item by item and Enter expands a tool call's output; in the input, up/down walk your prompt history. `/help` lists the rest. Permission prompts appear above the input; `y` allows once, `a` allows for the session, `n` denies.
