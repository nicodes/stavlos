...
Because humans can steer any agent, the runtime never guesses who a closing message is for: an agent answers another agent with `message` and answers the human in its normal reply. That rule is in every agent's system prompt.
