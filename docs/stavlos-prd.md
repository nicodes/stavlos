...
A **channel** is a root agent plus the subtree it spawns, bound to a working directory. Channels are the unit of independence: two channels bound to the same directory share nothing but the filesystem. Starting the CLI in a directory starts a channel; starting it again in the same directory starts another.
...
- A sidebar that is the swarm nav: channel directory, a swarm line (working / waiting counts) under the channel's tokens and cost, the agent tree with a needs-you badge and cost per row, `n` to jump to the next agent waiting on the human, and a folded channels section (this directory's other channels; space resumes one); the transcript shows the selected agent and the footer controls it — the permission and questions dialogs put the selected agent's prompt first and fall back to the oldest, the strip counts them all
