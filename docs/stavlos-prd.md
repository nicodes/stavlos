...
- A sidebar that is the swarm nav: session directory, a swarm line (working / waiting / needs-you counts, cost, age), the agent tree with a needs-you badge and cost per row, `n` to jump to the next agent waiting on the human; the transcript shows the selected agent and the footer controls it
