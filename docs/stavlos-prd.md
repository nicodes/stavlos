...
flowchart TD
    ...
    DAEMON --> CFG["Config files<br/><i>roles · skills · policy</i>"]
```
...
| Agent roles | YAML + markdown | Stable |
...
A **channel** is a root agent plus the subtree it spawns, with its own default working directory. Channels form a global catalog for the user's Stavlos data directory, independent of the shell's launch directory. Two channels using the same directory share only the filesystem. Names are unique across the daemon (`#name`), defaulting to the directory's base name with suffixes for collisions. Plain `stavlos` resumes the last human-viewed channel (or the newest active channel if that selection is unavailable); only an empty catalog creates an initial channel from the launch directory. `stavlos new [--dir path]` creates one and `stavlos open <#name>` resumes one by name. The TUI's new-channel dialog copies the current channel's directory into an editable field; this is a one-time default, never a link between channels.

`channel.set_dir {channel, dir}` changes an idle channel's default directory,
recorded as `channel.updated {dir}` and applied to the channel index in the same
transaction. It requires no active turns, background jobs, compaction or agent
prompts. The new project config and instructions are loaded, trust is reevaluated,
MCP processes stopped, remembered permissions cleared and mode reset to `ask`.
Explicit additional directories remain; configuration-derived directories are
resolved from the new default. History and channel/agent IDs remain intact.
Unavailable directories are exposed as `ChannelInfo.dir_error`, not silently
reassigned. Navigation selection is client state in `navigation.json`, separate
from agent activity. Sidebar and picker scope is global; draft/history state is
per channel, and late channel-specific RPC results are generation-checked.
...
Available to any agent whose role permits them:
...
**Background jobs** use the same mailbox and wake. There is one command tool, `shell`: it runs the command and waits up to a window (`wait`, default 15 seconds, max 300); a command that exits in time returns inline, one still running continues as a job — the call returns the job id and the output so far, the process is adopted by the agent runtime (`AdoptCommand`) and killed only at the job timeout (`timeout`, default one hour, max two) — and `background: true` starts it as a job without waiting, for servers and watchers. When a job exits the agent is woken with the exit code and output, and `shell_kill(id)` stops it. Every agent with `shell` has `shell_kill`. The window is a trade between an idle wait (bounded by the window) and a wake-up round trip (a whole model turn); 15 seconds keeps builds, small test runs and installs inline. Nothing is armed by hand: a job's exit always wakes its owner, as an agent's response always wakes the agent it answers. Jobs are logged (`job.started`, `job.finished`, `job.stopped`); a job whose process died with the daemon is reported to its owner as lost on restart. File watches and timers were tried and removed: models rarely used them well, and a background `sleep` or `inotifywait` covers the need.

In the TUI, permission, dirs and the selected agent's async, todo and mcp are permanent tabs with counts. Async lists the agents whose answers it expects (`awaiting` on `AgentInfo`), running jobs and replies owed; space on an agent row selects that agent. Tab focuses the strip, ←/→ move its highlight, and enter or a click opens the selected dialog, which closes back to its previous focus. Questions instead appear as inline chat messages: `? @main: Which format?` in channel chat and `? @user Which format?` in main's own chat. Addresses use a grey `@sender:` prefix followed by all recipients; the sender is omitted in its own chat, while channel chat treats the human as its viewpoint and omits the human recipient. Options use `□` and `■` glyphs and are indented below, along with the custom-answer field and Submit. All requested questions are published immediately in input order, each with its own prompt and submission. The human may answer in any order; accepting an answer updates only its message, keeping the same glyphs and indentation. The agent waits for the whole set and receives answers in original question order. Cancelling withdraws all outstanding prompts and preserves accepted answers in their original positions. Cards belong only to the viewed channel, never a global question queue, and arrival does not interrupt a draft. The event log retains the full question and structured selections/custom text for history and cross-client results.
...
**Todo list.** An agent whose role lists `todo` gets one `todo` tool, `todo(add?: [{text, status?}], update?: [{id, status?, text?}])`, which updates items and appends steps in one call and returns the whole list (a step may be added straight into `in_progress`, so a plan and its first step are one call, not two): a per-agent plan with items in `pending`, `in_progress`, `done` or `cancelled`. Every change is logged as `todo.changed` with the whole list, so recovery replays the last one and every client shows the same list; the current list is projected into the system prompt at every model call, so there is no read tool and the plan survives compaction. The tool descriptions carry the rules the field has converged on: three or more steps, exactly one item in progress, done only when verified, a new item for a blocker, and every change of a moment in one call rather than a run of them. It exists for the human watching: the TUI's "todo" tab counts done over total, its dialog lists the items with status glyphs, and the turn indicator names the item in progress. Newer models plan well without it (Claude Code, Codex and Gemini have all turned theirs off by default), so it is a role choice, on in `general`.

**Inline permissions.** Tool approvals use the same chat-card lifecycle as questions: `! @main: Permission: Shell` in channel chat, or `! @user Permission: Shell` in main's own chat, followed by the command or arguments and allow/deny choices. They appear immediately in both the channel chat and Discord without interrupting typing. In an agent's transcript they fold to one line, show all controls when highlighted or hovered, and use normal result folding after resolution. Denial reasons and alternate directories are edited inline in the TUI. Discord groups permission buttons on one row, offers immediate Deny and a separate Deny with reason popup, and offers boundary approval of the displayed directory only. Its directory follows the heading in regular text, and its shell command appears directly below without JSON. The full request, decision, reason and approved directory are logged; the original card updates in place whichever client answered. The `!` shortcut navigates to a card in the viewed channel. Project-config trust uses its separate dialog.

**The channel chat.** The TUI opens a channel on its chat (`docs/super-chat.md`): the human's posts, every agent's `message` to `user` (`chat.message`), questions and tool-permission cards, in the order they happen, with a loader naming the agents a post still waits on. Tool calls and notices stay in the agents' own chats, where they are kept for auditing; questions and permissions also appear there with the same inline controls and results. A post goes through `channel.post`: the @names at its front pick the agents, the rest is delivered to each as a `Steer` from the human exactly as written (the root gets it when it starts with no name), a leading name that is no live agent refuses the whole post, and logs it once on the channel as `chat.posted` with the names it went to. Parents are not told when the human messages their child. Selecting an agent in the sidebar opens its own chat, where typing sends `agent.send` to that agent as before.
...
- A sidebar that is the swarm nav: channel directory, the channel's tokens and cost, the agent tree with cost per row, and an orange `!` (permission) or `?` (question) in place of an agent's or channel's dot while the human is waited on. The `!` permission shortcut sits above the channels; directories are behind a gear (⚙) at each channel row's right edge (→ or clicking the gear opens its dirs dialog). Both controls return to the footer strip while the sidebar is hidden. Opening an agent waiting on a question or tool permission focuses its inline chat card. The permission shortcut counts pending approvals and navigates to a permission in the viewed channel; it does not open another channel's tool permission. `n` jumps to the next agent needing the human. The channel chat's footer shows channel mode and usage; an agent's chat adds role, model, variant and async · todo · mcp controls. Channels stay alphabetically ordered and retain their expanded agent trees when another is opened, refreshed through channel-aware `agent.tree` polling; ← folds a tree and space opens a channel. A `✚` beside the channels title creates a named channel in the default directory, refusing duplicate names. Channels reached inside the TUI open on their chat even when empty; the splash is only for a directory with no channel.
- Direct addressing of any agent in the tree with any envelope kind
- Permission and question prompts
- Model and role switching at any time: `/models` picks a model, `/roles` switches the selected agent's role in place (system prompt, tools, spawn list change at its next turn; logged as `agent.updated` and replayed on restart); `/mode` sets the channel's permission mode (logged as `channel.updated`, replayed on restart): **ask**, the default, prompts for every policy ask and every call outside the channel's working directories; **auto** (`/auto`) allows policy asks inside the channel's directories and denies calls outside them (the set grows in the dirs tab, or from a boundary prompt in ask mode); **yolo** (`/yolo`) allows everything a policy would ask about, boundary included. Switching to auto or yolo approves the waiting prompts that mode would have allowed (auto skips boundary prompts). Explicit deny rules, model questions and the project-trust prompt are unaffected in every mode. Auto matches Codex's Auto role and Claude Code's acceptEdits, with one honest difference: those are backed by a sandbox, this one by path inspection of commands (§10.7), so deny rules remain the floor. The TUI shows the mode tag at the start of the input, before its › (ASK dim, AUTO accent, YOLO orange), clickable back to ask; the mode is per channel rather than per agent (no guessing which subagents have it) and never daemon-wide (other channels are untouched); `/variants` picks a model variant — a provider-defined flavour such as reasoning effort (ChatGPT: low/medium/high/xhigh, Grok mini models: low/high) — per agent, logged as `agent.updated`, inherited by a child that inherits its parent's model, and shown after the model on the meta row; `/providers` connects a provider (§8.4)
...
Discord is implemented as a daemon-managed background service, controlled by
`/discord` in the TUI and `stavlos discord status|connect|disconnect` in a shell.
Its global `discord.enabled` setting is saved by connect/disconnect; daemon
shutdown preserves it for autoconnect. Status exposes bot/server names, channel
count and errors, never credentials. Closing a terminal leaves the integration
running. The current flat-channel, named-agent design is documented in
[discord-bridge.md](discord-bridge.md), with setup in
[discord-setup.md](discord-setup.md).

#### Early design notes (superseded by the bridge design above)

The original role/thread exploration is retained below for historical context.
...
1. Questions from `ask_user` and tool permissions are broadcast immediately to both `interactive` and `fallback` clients. Project-trust prompts initially go only to `interactive` clients. A client **claims** a prompt explicitly when the human acts; delivery alone never claims, so an unattended client cannot absorb prompts. The claimant is the only one that can answer. A claim with no `Reply` expires after a short timeout and the prompt is re-broadcast.
2. Project-trust prompts unclaimed after *N* seconds are broadcast to `fallback` clients as well. `PromptInfo.escalated` means visible to fallback clients; it is true from creation for questions and tool permissions, including in reconciliation snapshots.
...
- Model and role switching for a channel or agent
...
```jsonc
{
  ...
  // MCP servers reachable by roles that list them. Trust-gated at project level.
  "mcp": {
    "github": {
      "command": "npx",
...
A role (an "archetype" where a model is spoken to) is one markdown file: YAML frontmatter, then the system prompt. The filename is the role name and becomes the Discord role. Global roles live in `<config>/agents/`, project roles in `.stavlos/agents/`; a project role with the same name wins. Every key but `description` is optional, and anything unset is inherited.
...
Descriptions stay in context; bodies load only when an agent calls the `skill` tool, which is gated by the `skill` permission. Skills are scoped **per archetype** via the role's `skills:` list — otherwise every agent carries every description and progressive disclosure is defeated.

### 10.5 `AGENTS.md`

Plain markdown, model-facing prose rather than config — where "use light models for easy tasks" instructions live — read the way coding-agent harnesses have converged on (agents.md). Every agent in the channel follows it, subagents included, and no role can opt out. Each agent's system prompt carries, general first: your own `<config>/AGENTS.md`, then the repository's files from its git root down to the channel directory, one per directory — `AGENTS.md`, or `CLAUDE.md` where a directory has no `AGENTS.md` (a directory in no repository reads only its own, and the home directory never counts as a root). A later file is closer to the work and wins where they conflict, and the human's own messages win over all of them. The whole is bounded at 32 KiB: the file that crosses the budget is cut with a note, and later ones are left out. The instructions of the channel directory's subdirectories reach an agent when it first reads, searches or edits there: they are attached to that tool's result, once per agent, and again after a compaction has summarised the results that carried them (only in a trusted project, and never from hidden, dependency or build trees). An edit to any `AGENTS.md` or `CLAUDE.md` in a working directory asks in every mode, and the sandbox keeps the ones the trust hash knows read-only, so one agent cannot quietly rewrite what every other agent follows. The channel's other working directories are not read: their instructions would need a trust decision of their own.

### 10.6 Trust

Project configuration is data, but a cloned repository's data can still cause code to run: `mcp` server definitions start processes, policy `allow` rules loosen what the model may execute, and roles, skills, and `AGENTS.md` are instructions the model will follow. On first load of a directory, the daemon prompts once for **all of it** — the whole `.stavlos/` directory plus the instructions files agents will follow (§10.5: from the git root down to the channel directory, and those in its subdirectories) — showing what it contains, and records the decision keyed by directory plus a hash of those files' contents. A change to any of them re-prompts: an edited instructions file is noticed when an agent next starts a turn, and a file added later at the next load. Until confirmed, nothing from the project layer is loaded: no MCP servers start, no `allow` rules apply, no roles or skills are discovered, and only your own `AGENTS.md` reaches agents. The channel runs on global configuration alone, and the TUI and Discord both show that the project layer is pending trust.

A repository's files (`stavlos.json` and `stavlos.local.json` alike) use the same schema as the global one and take precedence over it once trusted: defaults, then global, then project, then local, key by key, with policy rules merged so that a later layer's rule for the same tool and pattern replaces an earlier one's, and lists (`dirs`, `hosts`, `env.pass`, `plugins`) adding to them. Both are in the trust hash, and the trust prompt is the only gate: trusting a repository means trusting everything its config can do — `allow` rules, the permission mode, passed environment variables, the search backend and its key, raised limits, an `allow` escalation default, and the sandbox itself.

### 10.7 Deliberately not in `.stavlos/`

- Plugin binaries — plugins are referenced from `stavlos.json`, in any layer (§11)
...
Plugins are listed in `stavlos.json`, like every other key: the global file's and a trusted project's add together. A project says `"model": "bedrock/…"`; if no installed plugin serves `bedrock`, the daemon fails at channel start with the install command. A project's `plugins` only apply once the project is trusted, since a plugin is a binary that runs.
...
Agents run commands against the channel's working directory. The daemon does not create worktrees or containers: which copy of a project an agent works in is left to the user and the model, and a role or skill can tell an agent to make a worktree first.
...
- MCP client
- Layered configuration (built-in defaults, global, project, local) with a trust gate on the project's; skills, roles, declarative policy
- Built-in tools: `grep` and `glob` (read-only search over contents and file paths, judged by path like `read` and allowed by default; they run ripgrep with an argument list they build, or walk the tree), `shell` (the one command tool; no command is allowed by default, since "read-only" commands such as `find -exec` or `rg --pre` run programs; a command outliving the wait window continues as a background job, and every command runs in the sandbox) and `shell_kill`, `web_fetch` and `web_search` (§6.5), `todo` (a per-agent plan, for roles that list `todo`), `read`, `apply_patch` (the Codex patch grammar: add, update with context-anchored hunks, delete, move; several files per patch, applied atomically), `skill`, the conversation set every agent has (`message`, `agent_status`), `ask_user` (one to four questions to the human, each its text and one to four options; every question is a checklist with multiple selections and an optional custom answer, so the model never adds an "Other"; all questions open immediately as independent prompts answerable in any order, never fall to the headless default and are unaffected by permission modes; the tool waits for all answers and returns "question → answer" lines in original question order, picks joined with ", "), and the lifecycle set for roles that spawn (`agent_create`, `agent_cancel`)
