...
**The session chat.** The TUI opens a session on its chat (`docs/super-chat.md`): the human's posts and every agent's `message` to `user` (`agent.message_to_user`), threaded under the post each answers; tool calls, permission prompts, questions and notices stay in the agents' own chats, where they are kept for auditing. A post goes through `session.post`: the daemon delivers it as a `Steer` from the human to every agent it @mentions (the root when it mentions none), refuses the whole post when a mention names no live agent, and logs it once on the session as `chat.posted` with the names it went to. Parents are not told when the human messages their child. Selecting an agent in the sidebar opens its own chat, where typing sends `agent.send` to that agent as before.
