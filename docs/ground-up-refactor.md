...
| 5.9 the protocol, layered | **the part that hurt** | A daemon of another build is replaced only when no agent is in a turn or running a job (`daemon.status` says how many are), or when `STAVLOS_RESTART_DAEMON=1` asks: any rebuild used to end every live turn. Splitting `protocol` into `wire` and `api/*`, and `daemonctl` out of `main`, are open. |
