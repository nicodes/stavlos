...
| 6.6 a keymap | **half** | Every key the TUI acts on is a binding in `keys.go` (the config editor and six letter keys compared the key's text), and a test refuses a handler that goes round them. Checked in a real terminal against a scratch daemon. Hints are still written by hand beside the bindings, and commands do not carry their `Run`. |
