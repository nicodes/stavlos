...
| 4.2 registry split | **half** | The plan-usage tracker is its own type with its own lock (`usage.go`) and the polling of usage endpoints its own file; the registry embeds the tracker. The provider table, credentials with refresh and the resolver still share `registry.go` and one mutex. |
