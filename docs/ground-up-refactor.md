...
| 6.2, 6.7 | open, on purpose | Moving the transcript's fold into `present` and having Discord and the browser draw its Items changes what three clients show from one place; each wants its own pull request, driven through a real terminal and a real Discord guild. What they build on is in place: the golden frames, Discord's cards pinned as JSON, `present`, the generated vocabulary. |
