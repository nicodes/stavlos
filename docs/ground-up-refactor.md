...
| Step | State | Notes |
|---|---|---|
| 0.1 golden behaviour | **done but the CLI** | 18 whole TUI frames (every card and dialog, the nav, the palette, the key bar, a narrow terminal, a channel loading), the model-facing prompt for a main agent and a child, tool definitions, Discord's nine prompt cards as the JSON the API receives, and the browser's reducer tests. CLI output is not pinned: every subcommand but `help` needs a daemon, and `startup_test.go` covers the part that has broken. |
...
| 0.3 no sleeping tests | **done for the negatives** | `Channel.quiet` (test-only) is the barrier: no agent in a turn or closing one, none with an input that starts one, no wake untaken. The nine "sleep, then see that nothing followed" checks in the agent package wait on it. The 32 sleeps left are poll intervals inside bounded wait loops, which cannot pass by luck, and four timing windows in the daemon and OAuth tests. |
...
| 3.1 `pathx`, `os.Root` | **done where it guards something** | `pathx.Rel`/`Within`/`Under` is the one containment test, at all seven sites, fuzzed. What is served to a browser (a sheet) is opened through an `os.Root`, so a file replaced by a link elsewhere is refused. The file tools keep resolving and then opening by path: their boundary is the permission decision on the resolved path plus the sandbox, and moving every tool onto roots would change which paths they accept (a working directory that is itself a link, files reached through links the human made), which is a behaviour change to decide, not a refactor. |
| 3.2 pure `Decide` | **done but the †** | `judge(facts)` is the verdict: the rules, what an allow rule cannot speak for (a compound command, a control file), what answers an ask (a permit, a listed host), then `ModeVerdict`, which the switch of mode over waiting prompts calls too. It reads nothing but its argument, and a test walks all 432 combinations for the promises the stages make (a deny is never loosened, a control-file edit is never allowed without a human, auto never lets a call out of the directories). Denying native tools the sandbox's hidden paths in every mode is † and waits for D3; permits keyed by principal wait for a second kind of principal. |
...
| 3.6 who may call what | **done but the token** | Scope is on the route (`owner ⊃ bridge ⊃ web`) and dispatch enforces it; a connection's caller is fixed when it is accepted, by its listener and its process, never by `attach`. The in-process Discord bridge is a `bridge` (the web's methods and five more), no longer the owner. An owner token in a 0600 file the sandbox hides is open: the socket is already 0600 and descendants of the daemon are refused, so it would add a second lock to the same door. |
| 3.7 sandbox level | **done** (D3–D5 answered 2026-09-20: all three on) | The level is on `ChannelInfo` and the nav shows it when it is less than full. Where the sandbox is wanted and the kernel offers none, every command asks, sticky like a control-file edit: no rule, permit or mode answers it. The file tools are refused the sandbox's hidden paths in every mode (`hiddenFrom`, the first stage of `judge`). A sticky prompt no longer offers "allow for this channel", which it never honoured. Still open from 1.7: placeholders bound read-only so a command cannot create an `AGENTS.md` that does not exist yet (withholding untrusted instructions already covers what that would buy). |
| 3.8 web listener | **done but one** | Sign-out ends the session's sockets; a sheet is served only to a frame; the socket wants a page key from `localStorage` beside the cookie, because a cookie is sent to every port of the host. Not done: taking the one-time code out of `xdg-open`'s `argv`. Every way to do it (a file the browser opens, a pairing code typed by hand) either breaks sandboxed browsers, which cannot read `~/.local` or `/run/user`, or changes how signing in looks: that is a decision (D11), not a refactor. The code is single-use and lasts two minutes. |
...
| 4.2 registry split | **done** | `registry.go` (833 lines) is the provider table and status (`registry.go`), `credentials.go` (sign-in, refresh, tokens), `resolve.go` (a model id to an adapter), `models.go` (what each plan may use), `usage.go` (the tracker, under its own lock) and `polling.go`. The registry's own mutex now guards only the adapters it has built and the single-flight refreshes. |
...
| 5.6 push, not poll | **done for the TUI** | A `changed` notification for the web UI, Discord and plan usage; the polls for them are a 30 s net. The channel list and the cache figure are still asked every 3 s (they change with every event, so a push would be as frequent), and the browser still polls its status. |
| 5.7 `Service` registry | **done** | `services.go`: Discord, the web listener, the plan poller, the recap loop and the watcher of Discord's status are one list, started in order once the socket listens and stopped in reverse as the first thing `Close` does; `Close` is ordered, runs once, and runs when a start fails. `service.*` methods were not added: `discord.*` and `web.*` are what clients call, and renaming them is a wire change with nothing to gain. |
| 5.9 the protocol, layered | **done, by file** | A daemon of another build is replaced only when no agent is working, or when `STAVLOS_RESTART_DAEMON=1` asks. `protocol` is `wire.go` (framing, errors), `api_channels.go`, `api_agents.go`, `api_prompts.go`, `api_catalog.go` and the shared vocabulary in `types.go`; finding, starting and replacing the daemon is `cmd/stavlos/daemonctl.go`, out of `main`. They stay one package each: separate packages would change the import path in some three hundred places and the name of every type on the wire for no change in what depends on what. `attach` returning capabilities waits for a second protocol version to need it. |
| 5.8 the daemon, split | **done** | `daemon.go` is five files, and the one lock is three: channels, clients (`hubMu`) and trust prompts (`trustMu`), none held across a call into a channel. Splitting them found one such call (`Trust` read each channel's directory under the daemon's lock, the loop the comment on `channelList` warns of); a commit's delivery to clients no longer touches the channels' lock at all. Small state stays in files written by `statefile`: moving it into the log's database would make a corrupt log lose the web UI's switch and the plan readings too. |
...
| 6.1 `present` | **done as a vocabulary** | What every client says the same way lives in one package with no dependency on any of them: the answers to a permission prompt, the argument that stands for a tool call (generated into the browser's types, with a test holding Discord's cards to it), what changed about an agent and why (`AgentChanges`: Discord used to drop a model the harness moved an agent to, with the reason), and who a message is for (`Addressed`, with the two rules for who goes without saying). |
| 6.6 a keymap | **done** | Every key the TUI acts on is a binding in `keys.go`, a test refuses a handler that goes round them, and the legend takes each key's name from its binding (`keyLabel`), so it cannot name a key a binding no longer has; the golden key bar did not change. What is left literal is what the text field handles itself (typing, `ctrl+j`, `/`). Commands carrying their `Run` is folded into 6.4's remainder. |
| 6.4 the TUI's model, split | **done but the pane stack** | `Model` is its parts (`channelState`, `uiPrefs`, `dialogs`, `promptState`, `navState`, `serviceState`, `chatPane`, `statusState`), each embedded, with the golden frames byte for byte the same. A command carries its run: the palette lists it, `commandRuns` runs it, a test holds them to each other, and the 30-branch switch (the most complex function in the TUI) is gone from the ratchet's baseline. One pane stack in place of the overlay, the tab dialogs and the config editor is not done: they differ in who owns the keyboard and what closes them, the frames now pin all three, and merging them is the kind of change to make with somebody watching the screen. |
| 6.2, 6.7 | **decided against as designed; done as 6.1** | The plan had every event fold into a neutral `Item` that the terminal, Discord and the browser all draw. Doing the work showed why they do not: the three are not three views of one list. The terminal keeps a transcript per agent with a cursor, folds, streaming and in-place replacement; Discord posts messages it can never rewrite, to a chat where tool calls are noise; the browser keeps rows keyed for a reactive list. A shared `Item` rich enough for the first is mostly ignored by the other two, and the daemon sending Items (D7) would put presentation on the wire. What they did share, and said differently, was vocabulary, and that is what `present` now owns. `transcript` (fold to lines) and `render` (lines to cells) were already separate packages. |
| 7 | **done but what is yours to decide** | The README is one sentence to a line. The PRD describes what exists: four providers and the traits table, in-tree adapters with Lua as the planned seam (the `go-plugin` design kept for reference), the sandbox as a shipped boundary and not a roadmap item, layered configuration over built-in defaults, the web UI and Discord as shipped. Left because they are visible or yours: the `plugins` config key and `stavlos plugin` stub (Lua plugins are planned, and an unknown key is an error, so removing it would break a config that has it), `KindPlugin` (the tests' provider), `IDToken` (in use), the standalone `stavlos-discord` binary (D5), renaming "preset" to "role" across 170 sites and the protocol (a wire change), and a LICENSE (which one is not mine to choose). |
