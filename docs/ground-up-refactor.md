...
| 6.2, 6.4, 6.7 | open, on purpose | The transcript split, the TUI's sub-models, and Discord and the browser drawing neutral Items rewrite how every screen is built. Each wants to be its own pull request, driven through a real terminal against a scratch daemon (`~/.cache/stavlos-probe`, isolated directories, never the live one). What they build on is in place: 18 golden frames, Discord's cards pinned as JSON, `present`, rows by id, the generated vocabulary, the complexity ratchet. |
