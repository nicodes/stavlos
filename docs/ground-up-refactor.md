...
| 6.4 the TUI's model, split | **the first step** | `Model` is its parts: `channelState`, `uiPrefs`, `dialogs`, `promptState` (which were there) and now `navState`, `serviceState`, `chatPane` and `statusState`, each embedded, so nothing that reads a field changed and all 18 golden frames are byte for byte the same. Giving each part pointer receivers and its own `Update`, and one pane stack in place of three modal systems, is open. |
