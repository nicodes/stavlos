...
| 5.8 the daemon, split | **by file** | `daemon.go` (827 lines) is `daemon.go` (start, stop, what it lends channels), `channels.go`, `trust.go`, `logins.go` and `hub.go` (clients, subscriptions, the handover from history to live). The state files already share `statefile`. One lock per part with no I/O under it, and small state moving into the log's database, are open: `d.mu` still guards channels, clients and trust prompts together. |
