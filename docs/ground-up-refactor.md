...
| 6.2, 6.4, 6.6, 6.7 | open, on purpose | The transcript split, the TUI's sub-models, the keymap and Discord and the browser drawing neutral Items rewrite how every screen is built. Six golden frames and the unit tests are not enough to call that unchanged: each wants to be its own pull request, driven through a real terminal against a scratch daemon (`STAVLOS_DATA_DIR` in a temp directory, never the live one), with golden frames for every dialog first (the rest of 0.1). What they build on is in place: `present`, rows by id, the generated vocabulary, the complexity ratchet (19 functions over 25, none may grow). |
