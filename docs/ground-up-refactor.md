...
| 7 | **done** (decisions answered 2026-09-20) | The README is one sentence to a line; the PRD describes what exists; an MIT LICENSE; the standalone `stavlos-discord` binary and its unit are gone; "preset" is "role" in the code, the protocol (`roles`, `RoleInfo`) and the docs, with `/presets` kept as a command alias and "archetype" kept where a model is spoken to (the prompt goldens did not change; one tool-argument description did). Kept: the `plugins` config key and `stavlos plugin` stub (Lua plugins are planned, and an unknown key is an error), `KindPlugin` (the tests' provider), `IDToken` (in use). |
