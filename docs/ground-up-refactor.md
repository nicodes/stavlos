...
| 6.4 the TUI's model, split | **done** | `Model` is its parts, each embedded, with the golden frames byte for byte the same; a command carries its run. `pane.go`: the configuration editor, an overlay and a tab's dialog are one interface (`key`, `click`, `hover`, `view`, and whether a miss is swallowed or the whole screen is taken), and `Model.pane()` is the one place that says which is on top. The key handler, the mouse handler, the view, the focus rules and `Update` each used to ask about all three by name, in an order each had to repeat. Checked in a real terminal against a scratch daemon: an overlay picks, a tab dialog opens, ctrl+c closes the editor and does not quit. |
