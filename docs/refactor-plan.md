...
- **Phase 4 (protocol): mostly done.** Typed method descriptors shared by client and daemon (client.Do, route); role and name on the wire; every channel id travels as channel; dead fields and the prompt record hook removed; a channel switched back to subscribes from the seq after what it kept. Still to do: coalesced stream deltas.
