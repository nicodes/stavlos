...
- **Phase 6 (TUI): mostly done.** The spinner ticks only while something animates; one frame computation decides where the chat and footer sit for layout, drawing and the mouse; model.go split by area into files (events, prompts, tabs, focus, input, meta, mouse, sidebar, chat navigation, pickers); one cursor, marker and scroll window (list.go) for the overlay, palette, @name list, tab dialogs and prompt dialogs; the last eight channels left keep their replay, so switching back replays only what was missed; replay skips drawing until it catches up. Still to do: the sidebar on the list component (its row arithmetic, and scrolling), tab descriptors instead of string surgery, viewport refresh from dirty flags at the end of Update, only the visible window of rows assembled, an index for untied calls (lastUntiedCall scans the transcript for every job, so replay is quadratic), a tuitest package for the duplicated test helpers.
- **Phase 7 (cleanup): started.** The PRD names the schema 6 events (job.*, agent.updated, channel.updated, chat.message, input.queued, turn.aborted, compaction.done, usage on assistant.message). Still to do: dead code from the audits, naming (the 45 `Channel` methods with receiver `s`, `metaYolo` for the tag that shows every mode), stale comments, README.
