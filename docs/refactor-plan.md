...
- **Phase 6 (TUI): done but one item.** The spinner ticks only while something animates; one frame computation decides where the chat and footer sit for layout, drawing and the mouse; model.go split by area into files; one cursor, marker and scroll window (list.go) for every list; the sidebar is one typed row list that keys, clicks and drawing index, and it scrolls; the strip's tabs are descriptors; events, stream deltas and ticks mark the chat dirty and Update redraws it once; the last eight channels left keep their replay; an index of call lines per tool makes replay linear; a tuitest package for the shared test helpers. Left open: assembling only the visible rows. The render cache already reuses unchanged items; what each redraw still pays is bubbles' viewport splitting the whole content and measuring every line's width (used only for horizontal scrolling, which the TUI never does), and avoiding that means replacing or forking the viewport.
- **Phase 7 (cleanup): mostly done.** The PRD names the schema 6 events; the README describes the sandbox, the search tools and the channel's directories; the Channel is `c` everywhere (receivers and fields, renamed with type information) and the mode tag is metaMode; ask.requested is logged once the prompt is open (a client could list prompts before it existed). Still to do: dead code the audits listed that staticcheck does not catch, stale comments.
