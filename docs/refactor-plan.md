...
- **Phase 4 (protocol): done.** Typed method descriptors shared by client and daemon (client.Do, route); role and name on the wire; every channel id travels as channel; dead fields and the prompt record hook removed; a channel switched back to subscribes from the seq after what it kept; an agent's stream deltas that continue each other go out as one notification within 30ms, and a channel's pending deltas go out before its events.
- **Phase 5 (tools, config, models): done.** One wire vocabulary for the two OpenAI-shaped adapters; web_fetch pages by characters; shared web transports; read limits as documented; one clip rule; a search backend table. A broken stream is sent again once (Reset delta, fresh codec). The defaults are one config.File, applied by LoadGlobal and written by stavlos init; the models.dev catalog decodes and caches only the providers Stavlos serves (a 4.6 MB cache becomes a few kilobytes).
- **Phase 6 (TUI): done.** The spinner ticks only while something animates; one frame computation decides where the chat and footer sit for layout, drawing and the mouse; model.go split by area into files; one cursor, marker and scroll window (list.go) for every list; the sidebar is one typed row list that keys, clicks and drawing index, and it scrolls; the strip's tabs are descriptors; events, stream deltas and ticks mark the chat dirty and Update redraws it once; the last eight channels left keep their replay; an index of call lines per tool makes replay linear; a tuitest package for the shared test helpers; render returns rows and chatViewport draws only the rows in view (BenchmarkChatRedraw, 2,000 items: 1.3 ms and 1.25 MB a redraw before, 0.47 ms and 0.65 MB after).
- **Phase 7 (cleanup): done.** The PRD names the schema 6 events; the README describes the sandbox, the search tools and the channel's directories; the Channel is `c` everywhere (receivers and fields, renamed with type information) and the mode tag is metaMode; ask.requested is logged once the prompt is open (a client could list prompts before it existed); the functions deadcode found unreachable are gone and test-only helpers live in the tests; every doc comment starts with the name it documents (a go/ast scan finds none left); background jobs are jobs in the protocol, the tools and the TUI (JobInfo without the kinds and states nothing produced).
