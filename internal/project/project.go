...
// ClearMinimum is the least a clearing must free to be worth doing: below
// it the history is left as it is, and the prompt cache with it.
const ClearMinimum = 20_000
...
// did. It is OpenCode's prune: nothing in the two most recent turns of the
// conversation (from the second-last user message on) is touched, since the
// model is still working from it; results of the tools that are the
// conversation itself (a child's answer, a message, the human's reply to a
// question) are never cleared, since they cannot be asked for again; and
// nothing is cleared unless at least minimum tokens would go, since a
...
func ClearOld(msgs []model.Message, keep, minimum int, keepTools map[string]bool) int {
	...
	// the message index where the second-last turn of the conversation starts
	recentFrom, turns := len(msgs), 0
	for i := len(msgs) - 1; i >= 0 && turns < 2; i-- {
		if msgs[i].Role == model.RoleUser && !onlyResults(msgs[i].Blocks) {
			turns++
			recentFrom = i
		}
	}
	recent, cleared := 0, 0
	var clear []*model.Block
	for i := recentFrom - 1; i >= 0; i-- {
