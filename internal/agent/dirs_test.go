...
func TestBashPathCandidates(t *testing.T) {
	...
	cases := []struct {
		...
		{"cat $STAVLOS_UNSET_VAR/etc/passwd", []string{"/etc/passwd"}}, // an unset variable expands to nothing, as in the shell
		{"cat ~root/.ssh/id_rsa", []string{homeOf("root") + "/.ssh/id_rsa"}},
	}
	for _, c := range cases {
...
// TestBashPathCandidatesFollowLinks: a relative argument that leads out of
// the working directory through a symlink is judged where it leads.
func TestBashPathCandidatesFollowLinks(t *testing.T) {
	...
	got := bashPathCandidates("cat link/secret plain.txt", base)
	real, _ := filepath.EvalSymlinks(outside)
	if !reflect.DeepEqual(got, []string{filepath.Join(real, "secret")}) {
		t.Fatalf("got %v", got)
	}
}
