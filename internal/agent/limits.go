...
func (a *Agent) reportTurnLimit(limit int) {
	text := fmt.Sprintf("%s (%s) reached its turn limit of %d without answering; message it again only if you raise the limit in its role, or kill it and delegate elsewhere.", a.Label, a.ID, limit)
