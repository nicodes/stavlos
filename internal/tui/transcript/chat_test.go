...
func TestChatKeepsOnlyPostsAndReplies(t *testing.T) {
	...
	if n := c.Items(); n != 3 {
		t.Fatalf("items %d, want post, permission and reply:\n%+v", n, c.All())
	}
	if got := chatItem(c, 0) + " || " + chatItem(c, 2); got != "@scout look around || @scout: Found it|>the bug is in `parse`" {
		t.Fatalf("chat %q", got)
	}
	if got := chatItem(c, 1); got != "@scout: Permission: Shell|>Allowed once" {
		t.Fatalf("permission result: %s", got)
	}
	for _, l := range c.All() {
		...
		if l.Lead && (l.Who != "user" || strings.Join(l.Names, ",") != "scout") {
...
func TestChatInArrivalOrder(t *testing.T) {
	...
	want := "@scout check the tests / what's the stack? / and the setup? / @main: Go 1.27 / @scout: All 42 pass."
